function [q,q_sim,out_param,qm] = cubSobolBayesian_IS_BAD(f1_IS,f2_IS,absTol,A,mu,d,x)
...
f1_IS = @(x) f1_IS(bsxfun(@plus,gail.stdnorminv(x)*A',mu));
f2_IS = @(x) f2_IS(bsxfun(@plus,gail.stdnorminv(x)*A',mu));
...
for m=out_param.mmin+1:out_param.mmax
   ...
   %lgp = 1./(1+1./expxb);  
    ynext1=f1_IS(xnext); %,lgp);
    yval1=[yval1; ynext1];
    ynext2=f2_IS(xnext); %,lgp);
    yval2=[yval2; ynext2];
