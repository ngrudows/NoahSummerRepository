function [q,q_sim,out_param,qm] = cubSobolBayesian_BAD(f1,f2,absTol,d,x)
...
lgp = 1./(1+1./expxb);
f1 = @(x,lgp) f1(norminv(x),lgp); %(gail.stdnorminv(x));
f2 = @(x,lgp) f2(norminv(x),lgp); %(gail.stdnorminv(x));
